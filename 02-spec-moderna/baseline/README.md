...
![ESTÁGIO 02 Spec Moderna](https://img.shields.io/badge/ESTÁGIO-02%20Spec%20Moderna-00A4EF?style=for-the-badge) ![TIPO Reference Bundle](https://img.shields.io/badge/TIPO-Reference%20Bundle-1A1A1A?style=for-the-badge) ![COUNT 12 docs](https://img.shields.io/badge/COUNT-12%20docs-7FBA00?style=for-the-badge)
...
This `baseline/` bundles four layers of that contract:
...
| **Bounded contexts** | How the system is partitioned (DDD), what each owns, how they communicate | [`bounded-contexts.md`](bounded-contexts.md) |
...
Together they answer the four questions Spec-Kit needs to operate:

1. **What technology are we building on?** → `techstack.md`
2. **What must the system do/be?** → `requirements.md`
3. **How is the system partitioned and where do features land?** → `bounded-contexts.md`
4. **Why are the non-obvious choices the way they are?** → ADRs
...
| [`bounded-contexts.md`](bounded-contexts.md) | 4 DDD contexts (BeneficiaryManagement, SocialProgramRegistry, PaymentProcessing, ReportingAndAudit) with carving criteria, communication matrix, ArchUnit rules | Scoping a feature, deciding which package new code belongs to |
...
techstack.md section 1, decision discipline from ADRs-INDEX.md, bounded
context boundaries from bounded-contexts.md, and non-negotiable constraints
from requirements.md section 4.
```

### 2. `/speckit.specify`

Reference the requirements, bounded context, and relevant ADRs for the feature being specified. Suggested prompt:

```text
/speckit.specify Beneficiary Registration. Input documents:
- Bounded context: 02-spec-moderna/baseline/bounded-contexts.md §1 (BeneficiaryManagement)
...
Pull tech stack, bounded contexts, and ADRs into the plan. Suggested prompt:

```text
/speckit.plan Use the stack defined in 02-spec-moderna/baseline/techstack.md.
Honor the architectural decisions in 02-spec-moderna/baseline/ADR-*. Place
code in the package defined by 02-spec-moderna/baseline/bounded-contexts.md.
...
3. **[`bounded-contexts.md`](bounded-contexts.md)** (15 min) — how the system is partitioned, where features land
4. **[`ADRs-INDEX.md`](ADRs-INDEX.md)** (5 min) — overview of decisions
5. **[`ADR-001`](ADR-001-modular-monolith.md)** (10 min) — foundational architecture
6. **[`ADR-003`](ADR-003-financial-rounding.md)** + **[`ADR-007`](ADR-007-pii-masking-policy.md)** (15 min) — financial and compliance critical decisions
7. **Remaining ADRs as needed** when working on the relevant context

Total: ~1.5 hours for full baseline; 45 min for the critical path.
...
| **Update `bounded-contexts.md`** (re-carve a context, add/remove a context) | New ADR justifying the carve change + PR review by Par 2 + impact analysis on existing specs |
...
| 4 bounded contexts (`discovery-report.final.md`) | ✅ Formalized in `bounded-contexts.md` with DDD criteria and ArchUnit enforcement |
...
- [`../scope-decisions.md`](../scope-decisions.md) — PO sign-off worksheet (migrar/descartar/evoluir)
